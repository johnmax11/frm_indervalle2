			...
			$header = <<<EOH
				...
				<!--[if gte mso 9]><xml>
				 <o:DocumentProperties>
				  <o:LastAuthor>Amada Colombia</o:LastAuthor>
...
EOH;
		...
		* @Return : Void
		*/
		function writeCol($value,$colspan=1){
			if($this->state!="OPENED")
			{
				$this->error="Error : Please open the file.";
				return false;
			}
                        $style_a = 'text-align:general;';
                        if(is_numeric($value)){
                            $style_a = 'text-align:right;';
                        }
                        if($colspan>1){
                            $style_a = 'text-align:center;';
                        }
                        if($this->_border){
                            $style_a .= " border-style:solid; border-width:1px; ";
                        }
                        
			fwrite($this->fp,"<td colspan=".$colspan." class=xl24 width=64 style='".$style_a."'>".(is_numeric($value)?(float)$value:$value)."</td>");
		}
	}
}